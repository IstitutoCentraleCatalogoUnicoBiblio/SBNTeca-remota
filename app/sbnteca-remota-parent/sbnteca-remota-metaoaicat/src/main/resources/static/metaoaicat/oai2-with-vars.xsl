...
<xsl:variable name="default_prefix">DEFAULT_PREFIX</xsl:variable>
<xsl:variable name="mag">MAG_ACTIVE</xsl:variable>
<xsl:variable name="mets">METS_ACTIVE</xsl:variable>
<xsl:variable name="mets-simple">METS_SIMPLE_ACTIVE</xsl:variable>
<xsl:variable name="oai_dc">OAIDC_ACTIVE</xsl:variable>
<xsl:variable name="pico">PICO_ACTIVE</xsl:variable>
<xsl:variable name="e_edm">EDM_ACTIVE</xsl:variable>
<xsl:variable name="cidoc_crm">CIDOC_CRM_ACTIVE</xsl:variable>
<xsl:variable name="tecaHome">TECA_HOME</xsl:variable>

<xsl:template match="/">
<html>
  <head>
    <title>MetaOaicat OAI-PMH Server: OAI 2.0 Request Results</title>
    <style><xsl:call-template name="style"/></style>
  </head>
  <body>
  <div class="container">
    <h1>MetaOaicat OAI-PMH Server: OAI 2.0 Request Results</h1>
...
<tr valign="top">
<td><a href="http://www.oclc.org/research/software/oai/cat.shtm"><img border="0" src="oaicat_icon.gif" alt="OAICat - An OAI-PMH v2 Repository Framework" width="120" height="60"/></a>
 &#160; &#160; <a href="http://www.openarchives.org"><img border="0"   height="60" src="http://www.openarchives.org/images/OA100.gif"/></a>
 &#160;&#160; <img border="0" src="CONTEXT_URLimages/Solr_Logo_on_white.png"  height="60"/>
 </td>
</tr>
<tr valign="top">
<td>Developed by Gruppometa S.r.l. Uses OaiCat and Solr. Version: BUILDNUMBER</td>
</tr>
</table>
</div>
    <br/>
...
<xsl:template name="quicklinks">
    <ul class="quicklinks">
      <xsl:if test="string-length($tecaHome) &gt; 0">
        <li><a><xsl:attribute name="href"><xsl:value-of select="$tecaHome"/></xsl:attribute>Teca</a> | </li>
      </xsl:if>
      <li><a href="CONTEXT_URL">Home</a> | </li>
      <li><a href="?verb=Identify">Identify</a> | </li> 
      <li><a><xsl:attribute name="href">?verb=ListRecords&amp;metadataPrefix=<xsl:value-of select='$default_prefix'/>
      </xsl:attribute>ListRecords</a> | </li>
      <li><a href="?verb=ListSets">ListSets</a> | </li>
      <li><a href="?verb=ListMetadataFormats">ListMetadataFormats</a> | </li>
      <li><a>
      	 <xsl:attribute name="href">?verb=ListIdentifiers&amp;metadataPrefix=<xsl:value-of select='$default_prefix'/></xsl:attribute>ListIdentifiers      	 
      	 </a>
      	 </li>
    </ul>
</xsl:template>
...
<xsl:template match="oai:header">
  ...
  <table class="values">
    ...
    <td class="value">
      ...
      <xsl:text> </xsl:text><a class="link" href="?verb=GetRecord&amp;metadataPrefix={$default_prefix}&amp;identifier={oai:identifier}"><xsl:value-of select="$default_prefix"/></a>
   <xsl:if test="$e_edm = 'true'">
      <xsl:text> </xsl:text><a class="link" href="?verb=GetRecord&amp;metadataPrefix=edm&amp;identifier={oai:identifier}">edm</a>
   </xsl:if>
   <xsl:if test="$cidoc_crm = 'true'">
      <xsl:text> </xsl:text><a class="link" href="?verb=GetRecord&amp;metadataPrefix=cidoc_crm&amp;identifier={oai:identifier}">cidoc_crm</a>
   </xsl:if>
   <xsl:if test="$pico = 'true'">
      <xsl:text> </xsl:text><a class="link" href="?verb=GetRecord&amp;metadataPrefix=pico&amp;identifier={oai:identifier}">pico</a>
   </xsl:if>
   <xsl:if test="$mag = 'true'">
      <xsl:text> </xsl:text><a class="link" href="?verb=GetRecord&amp;metadataPrefix=mag&amp;identifier={oai:identifier}">mag</a>
   </xsl:if>
   <xsl:if test="$mets = 'true'">
      <xsl:text> </xsl:text><a class="link" href="?verb=GetRecord&amp;metadataPrefix=mets&amp;identifier={oai:identifier}">mets</a>
   </xsl:if>
      <xsl:if test="$mets-simple = 'true'">
        <xsl:text> </xsl:text><a class="link" href="?verb=GetRecord&amp;metadataPrefix=mets-simple&amp;identifier={oai:identifier}">mets-simple</a>
      </xsl:if>
   <xsl:if test="$oai_dc = 'true'">
...
<xsl:template match="oai:setSpec">
  <xsl:variable name="mdpre"><xsl:choose><xsl:when test="//oai:request/@metadataPrefix"><xsl:value-of select="//oai:request/@metadataPrefix"/></xsl:when>
  <xsl:otherwise><xsl:value-of select="$default_prefix"/></xsl:otherwise></xsl:choose></xsl:variable>
  <tr><td class="key">setSpec</td>
  <td class="value"><xsl:value-of select="."/>
    <xsl:text> </xsl:text><a class="link" href="?verb=ListIdentifiers&amp;metadataPrefix={$mdpre}&amp;set={.}">Identifiers</a>
    <xsl:text> </xsl:text><a class="link" href="?verb=ListRecords&amp;metadataPrefix={$mdpre}&amp;set={.}">Records</a>
  </td></tr>
</xsl:template>
...
<!--  il resumptionToken pu' essere vuoto. I.S. 30-08-2011 -->
<xsl:if test="text()">
...
<xsl:template match="pico:record"  xmlns:pico="http://purl.org/pico/1.0/" >
    <h3>PICO Metadata (pico)</h3>
  <div class="xmlSource">
    <xsl:apply-templates select="." mode='xmlMarkup' />
  </div>
</xsl:template>

<xsl:template match="mag:metadigit"  xmlns:mag="http://www.iccu.sbn.it/metaAG1.pdf" >
    <h3>MAG Metadata (mag)</h3>
  <div class="xmlSource">
    <xsl:apply-templates select="." mode='xmlMarkup' />
  </div>
</xsl:template>

<xsl:template match="rdf:RDF[./crm:*]"  xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#"
...
<xsl:template match="rdf:RDF[./edm:*]"
		 xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#"
		 xmlns:edm="http://www.europeana.eu/schemas/edm/" >
    <h3>Europeana Metadata (edm)</h3>
  <div class="xmlSource">
    <xsl:apply-templates select="." mode='xmlMarkup' />
  </div>
</xsl:template>
