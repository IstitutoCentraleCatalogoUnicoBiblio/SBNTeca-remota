...
$(document).ready(function() {
	...
	resultTableColumns.push({"title": "Titolo", "className": "sortable", "type": "html", "width": "30%"});
	resultTableColumns.push({"title": "Anno / Numero parte", "className": "sortable", "type": "html", "width": "15%"});
	resultTableColumns.push({"title": "Estremi fasc. / Titolo parte", "className": "sortable", "type": "html"});
	resultTableColumns.push({"title": "Livello", "className": "sortable", "type": "html"});
	resultTableColumns.push({"title": "Pubbl.", "className": "sortable", "type": "html"});
	resultTableColumns.push({"title": "Tipo", "className": "sortable", "type": "html"});
	...
	$.ajax({
		url: restUrl + "searchMag?facetOnly",
		type: "POST",
		...
		"ajax": {
			...
			"dataSrc": function(json) {
				...
				for(i = 0; i < json.mag.length; i++) {
					...
					row.push(json.mag[i].documentFormat!=undefined?json.mag[i].documentFormat: "");
					row.push(
						"<a title=\"Vai al contenitore\" href=\"vfs-container?idDirectory="+encodeURIComponent(json.mag[i].project)+
						"&amp;idContainer="+
							encodeURIComponent(json.mag[i].idContainer) +"\"> <i class=\"icon-folder-open btn-icon\"></i></a><div class=\"icon_title_table\"> Contenitore </div>" +
						"<a title=\"Vai alla scheda di dettaglio del "+json.mag[i].documentFormat +"\" " +
									"onclick=\"detailMag('" + json.mag[i].id.replace(new RegExp('\\\\', 'g'), "\\\\") +
									 "','"+json.mag[i].documentFormat+"');\">" +
									"<i class=\"icon-list btn-icon\"></i></a><div class=\"icon_title_table\"> Dettaglio </div>" +
							 (window.location.href.indexOf("search") >= 0 ?
							  "<a onclick=\"editor('" + json.mag[i].id + "','"+json.mag[i].documentFormat+"');\" title=\"Modifica "+json.mag[i].documentFormat+"\">" +
						   			"<i class=\"icon-edit btn-icon\"></i></a><div class=\"icon_title_table\"> Modifica "+json.mag[i].documentFormat+" </div>" : ""));
...
function editor(magID, documentFormat) {
	$.ajax({
