Glizy.oop.declare("glizy.FormEdit.repeat", {
    ...
    initialize: function (element, glizyOpt, form, addBtnId, idParent) {
        ...
        if(!this.readOnly)
        {
          if (this.isCollapsable) {
              ...
              var rowhandler = this.sortable ? '<span class="GFERowHandler" title="' + GlizyLocale.Repeater.drag + '"></span>' : '';
                ...
                $rowContainer
                    .append('<div class="GFERowCollapsed"><div class="GFERowHeader">'+rowhandler+'<span class="GFERecordTitle">' + GlizyLocale.Repeater.record + ' <span class="GFERecordId">1</span> <span class="GFERecordLabel"></span></span><span class="GFERowPreview"></span></div><div class="GFERowPanel"><i class="icon icon-pencil GFERowEdit" /> <i class="icon icon-trash GFERowDelete" /></div><div class="GFESideClearer"></div></div>')
              ...
              $rowContainer
                  .append(rowhandler + '<i class="icon icon-trash GFERowDelete GFERightIcon" ></i>')
          }
        }
        ...
        $(this.$element).off("click").on('click', '.GFERowDoCollapse', function () {
            ...
            if (hasConfirmed) {
                ...
                $rowCont.removeData('justCreated');
                var orId = $rowCont.data('originalId');
                self.children = _.without(self.children,_.findWhere(self.children,{"originalId":orId}));
        ...
        $(this.$element).on('click', '.GFERowEdit', function () {
            ...
            var eleScroll = $container.find(".GFERowExpanded");
            $(eleScroll).animate({scrollTop: 0});
            ...
            var msg = {
                "childIndex": _.findIndex(self.children,{"originalId":self.childActive})
            };
            Glizy.events.broadcast("glizy.formEdit2.repeat.openCollapsed",msg);
        });
    ...
    addDeleteHandler : function(containerId) {
        ...
        $('#'+containerId+' .GFERowDelete').on('click', function () {
    ...
    addRow: function (fieldSet, footer, id, justCreated, noVerifySelectWithTarget, value, newRow) {
        ...
        if(newRow){
            var msg = {
                "childIndex": id,
                "childValue": null,
                "elementId": fieldSetId
            };
            Glizy.events.broadcast("glizy.formEdit2.repeat.createdRepeat",msg);
        }
        ...
        if (value && value.length > 0) {
            ...
            for (var i in value) {
                ...
                var msg = {
                    ...
                    "childValue": value[i],
                    "elementId": this.$element[0] ? this.$element[0].id : null
                };
                Glizy.events.broadcast("glizy.formEdit2.repeat.createdRepeat",msg);
            }
            if (self.maxRec && value.length == self.maxRec) {
                $footer.find(".GFEAddRow").addClass('GButtonDisabled').attr('disabled', 'disabled');
            }
        }
    },
            ...
            for (var i in this.children) {
                ...
                for (var field in row) {
                    if(field!=="originalId"){
                        ...
                        if (!fieldObj.isDisabled() && !fieldObj.isValid()) {
